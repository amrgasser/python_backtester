...
def retrieve_data(table_name="AAPL", start="2018-12-17", end="2021-02-12"):
    """
    ...
    conn = psycopg2.connect(host="localhost", port=5432, database="stock",
                            user="postgres", password="root")  # password not needed for now
    cur = conn.cursor()
    ...
    for date in dates:
        ...
        #     [date])
        test_date = "2022%"
        cur.execute("SELECT open, date::varchar FROM %s" %
                    (table_name))
